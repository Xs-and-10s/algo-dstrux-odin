...
Interval :: struct($T: typeid) where T: ordered {
    low: T,
    high: T,
}

// IntervalTreeNode represents a node in an interval tree
IntervalTreeNode :: struct($T: typeid) where T: ordered {
    interval: Interval(T),
    max: T,
...
IntervalTree :: struct($T: typeid) where T: ordered {
...
interval_tree_make :: proc($T: typeid, allocator := context.allocator) -> IntervalTree(T) where T: ordered {
...
_interval_tree_insert_node :: proc(tree: ^IntervalTree($T), node: ^IntervalTreeNode(T), interval: Interval(T)) -> ^IntervalTreeNode(T) {
    if node == nil {
...
interval_tree_search :: proc(tree: ^IntervalTree($T), interval: Interval(T)) -> (Interval(T), bool) {
    return _interval_tree_search_node(tree.root, interval)
}

_interval_tree_search_node :: proc(node: ^IntervalTreeNode($T), interval: Interval(T)) -> (Interval(T), bool) {
    if node == nil {
        return {}, false
    }
