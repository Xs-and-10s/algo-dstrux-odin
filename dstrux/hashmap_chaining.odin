...
hashmap_chaining_make :: proc($K, $V: typeid, capacity: int = 16, allocator := context.allocator) -> HashMapChaining(K, V) {
...
hashmap_chaining_insert :: proc(hm: ^HashMapChaining($K, $V), key: K, value: V) where K: comparable {
...
hashmap_chaining_get :: proc(hm: ^HashMapChaining($K, $V), key: K) -> (V, bool) where K: comparable {
    index := hashmap_chaining_hash(hm, key)
    entry := hm.buckets[index]
...
hashmap_chaining_remove :: proc(hm: ^HashMapChaining($K, $V), key: K) -> bool where K: comparable {
    index := hashmap_chaining_hash(hm, key)
    entry := hm.buckets[index]
...
hashmap_chaining_contains :: proc(hm: ^HashMapChaining($K, $V), key: K) -> bool where K: comparable {
...
hashmap_chaining_rehash :: proc(hm: ^HashMapChaining($K, $V)) where K: comparable {
