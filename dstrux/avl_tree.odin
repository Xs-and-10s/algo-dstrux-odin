...
avl_make :: proc($T: typeid, allocator := context.allocator) -> AVLTree(T) {
	return AVLTree(T){root = nil, size = 0, allocator = allocator}
}
...
avl_insert :: proc(tree: ^AVLTree($T), value: T) {
	tree.root = _avl_insert_node(tree, tree.root, value)
}

_avl_insert_node :: proc(tree: ^AVLTree($T), node: ^AVLNode(T), value: T) -> ^AVLNode(T) {
	if node == nil {
...
avl_search :: proc(tree: ^AVLTree($T), value: T) -> bool {
	return _avl_search_node(tree.root, value)
}

_avl_search_node :: proc(node: ^AVLNode($T), value: T) -> bool {
...
avl_remove :: proc(tree: ^AVLTree($T), value: T) -> bool {
	old_size := tree.size
	tree.root = _avl_remove_node(tree, tree.root, value)
	return tree.size < old_size
}

_avl_remove_node :: proc(tree: ^AVLTree($T), node: ^AVLNode(T), value: T) -> ^AVLNode(T) {
