...
dll_make :: proc($T: typeid, allocator := context.allocator) -> DoublyLinkedList(T) {
	return DoublyLinkedList(T){head = nil, tail = nil, length = 0, allocator = allocator}
}
...
dll_find :: proc(list: ^DoublyLinkedList($T), value: T) -> ^DoublyLinkedListNode(T) {
	current := list.head
	for current != nil {
...
dll_remove :: proc(list: ^DoublyLinkedList($T), value: T) -> bool {
	current := list.head
	for current != nil {
		if current.value == value {
			if current.prev != nil {
