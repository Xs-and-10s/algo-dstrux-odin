...
sll_make :: proc($T: typeid, allocator := context.allocator) -> SinglyLinkedList(T) {
	return SinglyLinkedList(T){head = nil, tail = nil, length = 0, allocator = allocator}
}
...
sll_find :: proc(list: ^SinglyLinkedList($T), value: T) -> ^SinglyLinkedListNode(T) {
	current := list.head
	for current != nil {
...
sll_remove :: proc(list: ^SinglyLinkedList($T), value: T) -> bool {
	if list.head == nil {
		return false
	}
