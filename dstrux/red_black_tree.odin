...
rbt_make :: proc($T: typeid, allocator := context.allocator) -> RedBlackTree(T) {
	...
	return RedBlackTree(T){root = nil_node, nil_node = nil_node, size = 0, allocator = allocator}
}
...
rbt_insert :: proc(tree: ^RedBlackTree($T), value: T) {
...
rbt_search :: proc(tree: ^RedBlackTree($T), value: T) -> bool {
	node := tree.root
