...
minmax_heap_make :: proc($T: typeid, capacity: int = 16, allocator := context.allocator) -> MinMaxHeap(T) {
    return MinMaxHeap(T) {
...
minmax_heap_push :: proc(heap: ^MinMaxHeap($T), value: T) where T: ordered {
...
minmax_heap_pop_min :: proc(heap: ^MinMaxHeap($T)) -> (T, bool) where T: ordered {
    if len(heap.data) == 0 {
        return {}, false
    }
...
minmax_heap_pop_max :: proc(heap: ^MinMaxHeap($T)) -> (T, bool) where T: ordered {
    n := len(heap.data)
    if n == 0 {
        return {}, false
    }
...
minmax_heap_peek_max :: proc(heap: ^MinMaxHeap($T)) -> (T, bool) where T: ordered {
    n := len(heap.data)
    if n == 0 {
        return {}, false
    }
...
_minmax_heap_push_up :: proc(heap: ^MinMaxHeap($T), index: int) where T: ordered {
...
_minmax_heap_push_up_min :: proc(heap: ^MinMaxHeap($T), index: int) where T: ordered {
    idx := index
    for idx > 2 {
        grandparent := (idx - 3) / 4
...
_minmax_heap_push_up_max :: proc(heap: ^MinMaxHeap($T), index: int) where T: ordered {
    idx := index
    for idx > 2 {
        grandparent := (idx - 3) / 4
        if heap.data[idx] > heap.data[grandparent] {
...
_minmax_heap_push_down_min :: proc(heap: ^MinMaxHeap($T), index: int) where T: ordered {
...
_minmax_heap_push_down_max :: proc(heap: ^MinMaxHeap($T), index: int) where T: ordered {
